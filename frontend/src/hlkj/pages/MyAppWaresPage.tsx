...
export const myAppwaresSearchSchema = z.object({
  page: z.number().catch(1),
})

const PER_PAGE = 5

function getMyAppWaresQueryOptions({ page }: { page: number }) {
  return {
    queryFn: () =>
      AppWaresService.readMyAppWares({ 
...
interface MyAppWaresTableProps {
  routeFullPath: string
  page: number
}

function MyAppWaresTable({ routeFullPath, page }: MyAppWaresTableProps) {
  const navigate = useNavigate({ from: routeFullPath })

  const { data, isLoading, isPlaceholderData } = useQuery({
    ...getMyAppWaresQueryOptions({ page }),
      ...
      <EmptyState.Root>
        <EmptyState.Content>
          ...
          <VStack textAlign="center">
            <EmptyState.Title>您还没有创建任何应用仓库</EmptyState.Title>
...
export function MyAppWaresPage({ routeFullPath, page }: MyAppWaresTableProps) {
  return (
    <Container maxW="full">
      ...
      <AddAppWare />
      <MyAppWaresTable routeFullPath={routeFullPath} page={page} />
    </Container>
  )
}